...
#  deploy: install -m 755 acme-renew.sh.example /usr/local/bin/acme-renew.sh
...
# ---------------- 按需修改 ----------------
DOMAIN=ssl.example.com          # 证书域名
CERTDIR=/etc/ssl/securev/$DOMAIN
ACME=/root/.acme.sh/acme.sh
DNS_ENV=/etc/ocserv/acme-dns.env   # DNS-01 凭据(存在则加载)
LOG=/var/log/acme-renew.log
RELOAD_SERVICES="ocserv"        # 续期后需要 reload 的服务(热重载证书,不断线)
RESTART_SERVICES="ocserv-panel" # 续期后需要 restart 的服务(重新读取证书文件)
# -----------------------------------------
